  ...
  actions: {
    async listStreams(ivsRegion) {
      ...
      try {
        const response = await api.get(
          `https://${commonStore.apiIds[ivsRegion].rest}.execute-api.${ivsRegion}.amazonaws.com/ivs/list-streams`,
          {
            params: {
    ...
    async getSession(streamId, channelArn, ivsRegion) {
      try {
        ...
        const response = await api.get(
          `https://${commonStore.apiIds[ivsRegion].rest}.execute-api.${ivsRegion}.amazonaws.com/ivs/get-session`,
          {
            params: {
              stream_id: streamId,
    ...
    async getStream(streamId, channelArn, ivsRegion) {
      try {
        ...
        const response = await api.get(
          `https://${commonStore.apiIds[ivsRegion].rest}.execute-api.${ivsRegion}.amazonaws.com/ivs/get-stream`,
          {
            params: {
    ...
    async getIngestMetrics(streamId, channelId, ivsRegion) {
      try {
        ...
        const response = await api.get(
          `https://${commonStore.apiIds[ivsRegion].rest}.execute-api.${ivsRegion}.amazonaws.com/ivs/get-ingest-metrics`,
          {
            params: {
              stream_id: streamId,
    ...
    async getSessionEvents(streamId, channelArn, ivsRegion) {
      try {
        const ws = new WebSocket(
          `wss://${commonStore.apiIds[ivsRegion].wss_get_session_events}.execute-api.${ivsRegion}.amazonaws.com/ivs`
        );
        ws.onopen = () => {
    ...
    async getLiveStreams(ivsRegion) {
      try {
        const ws = new WebSocket(
          `wss://${commonStore.apiIds[ivsRegion].wss_get_live_streams}.execute-api.${ivsRegion}.amazonaws.com/ivs`
        );
        ws.onopen = () => {
          console.log("open response:", ws);
    ...
    getQuotaProvisioned(serviceCode, ivsRegion) {
      try {
        ...
        api
          .get(
            `https://${commonStore.apiIds[ivsRegion].rest}.execute-api.${ivsRegion}.amazonaws.com/ivsQuotaProvisioned`,
            {
              params: {
